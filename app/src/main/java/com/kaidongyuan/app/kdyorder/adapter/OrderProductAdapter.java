...
import com.kaidongyuan.app.kdyorder.ui.activity.MakePreOrderActivity;
...
public class OrderProductAdapter extends BaseExpandableListAdapter implements View.OnClickListener {
    ...
    public View getGroupView(int groupPosition, boolean isExpanded, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            groupViewHolder.textViewBigUomQty = (TextView) convertView.findViewById(R.id.tv_big_uom_qty);
            groupViewHolder.linearLayoutKucun = (LinearLayout) convertView.findViewById(R.id.ll_productInventory);
            groupViewHolder.linearLayoutBigUomQty = (LinearLayout) convertView.findViewById(R.id.ll_big_uom);
        ...
        //设置是否显示促销信息栏
        if (mContext instanceof MakePreOrderActivity){
            groupViewHolder.linearLayoutPromotion.setVisibility(View.GONE);
        }else if (getPolicyList(product.getPRODUCT_POLICY()).size() == 0) {
            groupViewHolder.linearLayoutPromotion.setVisibility(View.GONE);
        } else {
        ...
        // 有小单位就显示
        if (Tools.hasBASE_RATE(product.getBASE_RATE()) && product.getCHOICED_SIZE() > 0) {
            groupViewHolder.linearLayoutBigUomQty.setVisibility(View.VISIBLE);
            double bigqty = product.getCHOICED_SIZE() / product.getBASE_RATE();
            DecimalFormat df;
            if(product.getCHOICED_SIZE() % product.getBASE_RATE() == 0) {
                df = new DecimalFormat("#0");
            }else {
                df = new DecimalFormat("#0.00");
            }
            groupViewHolder.textViewBigUomQty.setText(String.valueOf(df.format(bigqty) + product.getPACK_UOM()));
        } else {
            groupViewHolder.linearLayoutBigUomQty.setVisibility(View.GONE);
        }
...
//        }
    ...
    private class GroupViewHolder {
        ...
        TextView textViewBigUomQty;//大单位数量
        LinearLayout linearLayoutBigUomQty; //大单位View
    }
