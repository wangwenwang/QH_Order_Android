...
import com.kaidongyuan.app.kdyorder.model.InputInventoryActivityBiz;
 ...
 */
public class InputInventoryActivity extends BaseActivity implements View.OnClickListener, SwipeRefreshLayout.OnRefreshListener, AdapterView.OnItemClickListener{
    /**
     ...
     * 对应的业务类
     */
    private InputInventoryActivityBiz mBiz;
    ...
    private void initData() {
        try {
            ...
            mBiz = new InputInventoryActivityBiz(this);
            ...
            if (intent.hasExtra(EXTRAConstants.OUTPUT_ORDER_TYPE)){
                strInputOrderType=intent.getStringExtra(EXTRAConstants.OUTPUT_ORDER_TYPE);
            }
    ...
    private void initView() {
        try {
            ...
            if (strInputOrderType!=null&&strInputOrderType.equals("input_return")){
                ...
                mOutPutToPartyAddress="其它入库客户地址信息";
    ...
    private void confirmOrder() {
        try {
            ...
            Intent intent = new Intent(this, InPutOrderConfirmActivity.class);
            ...
            if (strInputOrderType!=null){
                intent.putExtra(EXTRAConstants.OUTPUT_ORDER_TYPE,strInputOrderType);
            }
    ...
    public void getPayTypeDataSuccess() {//支付类型获取成功后获取产品类型和品牌类型
        try {
            if (!mBiz.getProductTypesData(strInputOrderType!=null&&strInputOrderType.equals("input_other"))) {
...
//                        if (strInputOrderType!=null&&strInputOrderType.equals("output_return")){
...
//                        }
    ...
    private static class InnerHandler extends Handler {
        private WeakReference<InputInventoryActivity> makeOrderActivityWeakReference;

        public InnerHandler(InputInventoryActivity mActivity) {
        ...
        public void handleMessage(Message msg) {
            try {
                ...
                InputInventoryActivity activity = makeOrderActivityWeakReference.get();
                if (activity != null) {
                    switch (msg.what) {
         ...
         */
        private void choiceProductDetialIn(Message msg, InputInventoryActivity activity) {
            try {
                int paddingTopIn = msg.arg1 - InputInventoryActivity.DETIAL_PICE_PADDING_SIZE_IN;
                activity.mRlChoiceProductdetial.setPadding(0, paddingTopIn, 0, 0);
                if (paddingTopIn > 0) {
                    Message message = activity.mHandler.obtainMessage();
                    message.what = InputInventoryActivity.WHAT_CHOICE_DETIAL_IN;
                    message.arg1 = paddingTopIn;
                    activity.mHandler.sendMessageDelayed(message, InputInventoryActivity.DETIAL_IN_PICE_TIME);
                } else {
         ...
         * @param activity MakeOrderActivity
         */
        private void choiceProductDetialOut(Message msg, InputInventoryActivity activity) {
            try {
                int paddingTopOut = msg.arg1 + InputInventoryActivity.DETIAL_PICE_PADDING_SIZE_OUT;
                activity.mRlChoiceProductdetial.setPadding(0, paddingTopOut, 0, 0);
                if (paddingTopOut < InputInventoryActivity.SCREEN_HEIGHT) {
                    Message messageOut = activity.mHandler.obtainMessage();
                    messageOut.what = InputInventoryActivity.WHAT_CHOICE_DETIAL_OUT;
                    messageOut.arg1 = paddingTopOut;
                    activity.mHandler.sendMessageDelayed(messageOut, InputInventoryActivity.DETIAL_OUT_PICE_TIME);
                } else {
