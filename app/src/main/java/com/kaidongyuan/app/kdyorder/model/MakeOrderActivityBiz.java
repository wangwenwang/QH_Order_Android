 ...
 * 下单界面业务类
 */
public class MakeOrderActivityBiz {
        ...
        try {
            StringRequest request = new StringRequest(Request.Method.POST, URLCostant.GET_PAYMENT_TYPE, new Response.Listener<String>() {
                ...
                public void onResponse(String response) {
                    Logger.w(MakeOrderActivityBiz.this.getClass() + "getPayTypeDataSuccess:" + response);
                    getPayTypeDataSuccess(response);
                }
            }, new Response.ErrorListener() {
                @Override
                public void onErrorResponse(VolleyError error) {
                    Logger.w(MakeOrderActivityBiz.this.getClass() + "getPayTypeDataVolleyError:" + error.toString());
    ...
    public boolean getProductTypesData() {
        try {
            StringRequest request = new StringRequest(Request.Method.POST, URLCostant.GET_PRODUCT_TYPE, new Response.Listener<String>() {
                @Override
                public void onResponse(String response) {
                    Logger.w(MakeOrderActivityBiz.this.getClass() + "getProductTypesDataSuccess:" + response);
                    getProductTypesDataSuccess(response);
                }
            }, new Response.ErrorListener() {
                @Override
                public void onErrorResponse(VolleyError error) {
                    Logger.w(MakeOrderActivityBiz.this.getClass() + "getProductTypesDataVolleyError:" + error.toString());
                    if (NetworkUtil.isNetworkAvailable()) {
    ...
    public boolean getProductsData(final String mOrderPartyId, final String mOrderAddressIdx, int index) {
        try {
            ...
            StringRequest request = new StringRequest(Request.Method.POST, URLCostant.GET_PRDUCT_LIST_TYPE, new Response.Listener<String>() {
                @Override
                public void onResponse(String response) {
                    Logger.w(MakeOrderActivityBiz.this.getClass() + "getProductsDataSuccess:" + response);
                    getProductsDataSuccess(response);
                }
            }, new Response.ErrorListener() {
                @Override
                public void onErrorResponse(VolleyError error) {
                    Logger.w(MakeOrderActivityBiz.this.getClass() + "getProductsDataVolleyError:" + error.toString());
                    if (NetworkUtil.isNetworkAvailable()) {
    ...
    public void addProductSizeInChoicedProducts(int dataIndex) {
        try {
            ...
            if(Tools.hasBASE_RATE(product.getBASE_RATE())) {
                size = product.getCHOICED_SIZE() + (int)product.getBASE_RATE();
            }else{
                size = product.getCHOICED_SIZE() + 1;
            }
        ...
        try {
            if (index != -1) {
                ...
                if(Tools.hasBASE_RATE(product.getBASE_RATE())) {
                    ...
                    if (choiceSize >= product.getBASE_RATE()) {
                        product.setCHOICED_SIZE(choiceSize - (int)product.getBASE_RATE());
                    }
