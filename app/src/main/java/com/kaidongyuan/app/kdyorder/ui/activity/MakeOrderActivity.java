 ...
 * 下单界面
 */
public class MakeOrderActivity extends BaseActivity implements View.OnClickListener, SwipeRefreshLayout.OnRefreshListener, AdapterView.OnItemClickListener {
     ...
     * 对应的业务类
     */
    private MakeOrderActivityBiz mBiz;
    ...
    private void initData() {
        try {
            ...
            mBiz = new MakeOrderActivityBiz(this);
    ...
    private void initView() {
        try {
            ...
            mOrderProductAdapter = new OrderProductAdapter(this, null, null);
    ...
    private void confirmOrder() {
        try {
            ...
            for (int i = 0; i < mBiz.getChoiceProducts().size(); i++) {
                ...
                if(Tools.hasBASE_RATE(m.getBASE_RATE())) {
    ...
    private void showConfirmDialog() {
        try {
            ...
            if (mConfirmCustomerInformationDialog == null) {
                ...
                window.findViewById(R.id.linearlayout_choicepaymenttype).setOnClickListener(this);
    ...
    private static class InnerHandler extends Handler {
        private WeakReference<MakeOrderActivity> makeOrderActivityWeakReference;

        public InnerHandler(MakeOrderActivity makeOrderActivity) {
            this.makeOrderActivityWeakReference = new WeakReference<>(makeOrderActivity);
        }
        ...
        public void handleMessage(Message msg) {
            try {
                ...
                MakeOrderActivity activity = makeOrderActivityWeakReference.get();
                if (activity != null) {
                    switch (msg.what) {
         ...
         */
        private void choiceProductDetialIn(Message msg, MakeOrderActivity activity) {
            try {
                int paddingTopIn = msg.arg1 - MakeOrderActivity.DETIAL_PICE_PADDING_SIZE_IN;
                activity.mRlChoiceProductdetial.setPadding(0, paddingTopIn, 0, 0);
                if (paddingTopIn > 0) {
                    Message message = activity.mHandler.obtainMessage();
                    message.what = MakeOrderActivity.WHAT_CHOICE_DETIAL_IN;
                    message.arg1 = paddingTopIn;
                    activity.mHandler.sendMessageDelayed(message, MakeOrderActivity.DETIAL_IN_PICE_TIME);
                } else {
         ...
         * @param activity MakeOrderActivity
         */
        private void choiceProductDetialOut(Message msg, MakeOrderActivity activity) {
            try {
                int paddingTopOut = msg.arg1 + MakeOrderActivity.DETIAL_PICE_PADDING_SIZE_OUT;
                activity.mRlChoiceProductdetial.setPadding(0, paddingTopOut, 0, 0);
                if (paddingTopOut < MakeOrderActivity.SCREEN_HEIGHT) {
                    Message messageOut = activity.mHandler.obtainMessage();
                    messageOut.what = MakeOrderActivity.WHAT_CHOICE_DETIAL_OUT;
                    messageOut.arg1 = paddingTopOut;
                    activity.mHandler.sendMessageDelayed(messageOut, MakeOrderActivity.DETIAL_OUT_PICE_TIME);
                } else {
                    activity.mRlChoiceProductdetial.setVisibility(View.GONE);
                    Logger.w("AddGiftActivity:已选商品详细列退出动画结束");
                }
            } catch (Exception e) {
                ExceptionUtil.handlerException(e);
            }
